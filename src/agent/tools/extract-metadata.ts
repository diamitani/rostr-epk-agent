...
export async function formatInputs(intake: EPKIntake): Promise<{
  ...
  const sections: string[] = [
    ...
    `- **Date of birth:** ${intake.date_of_birth || "unknown"}`,
    `- **Birth city:** ${intake.birth_city || "unknown"}`,
    `- **Current city:** ${[intake.city, intake.state, intake.country].filter(Boolean).join(", ")}`,
    `- **Hometown:** ${intake.hometown || "same as birth city"}`,
    `- **Genre:** ${intake.genre}${intake.genre_additional?.length ? ` (also: ${intake.genre_additional.join(", ")})` : ""}`,
    `- **Subgenre:** ${intake.subgenre || "unknown"}`,
    `- **Artist type:** ${intake.artist_type?.length ? intake.artist_type.join(", ") : "unknown"}`,
    `- **Year started:** ${intake.year_started || "unknown"}`,
    `- **P.R.O.:** ${intake.pro_affiliation || "not registered / not provided"}`,
    ...
    ``,
    `### Music Theme / Style (artist's own words)`,
    `${intake.music_theme_notes || "none provided"}`,
    ``,
    `### Artist Identity / Brand`,
    `${intake.artist_identity_brand || "none provided"}`,
    ``,
    `### Goals`,
    ...
    `### Past Collaborations`,
    `${intake.past_collaborations || "none provided"}`,
    ``,
    `### Performances`,
    `${intake.performances || "none provided"}`,
    ``,
    `## Riders`,
    `### Technical Rider`,
    `${intake.technical_rider || "none provided"}`,
    ``,
    `### Performance Rider`,
    `${intake.performance_rider || "none provided"}`,
    ``,
